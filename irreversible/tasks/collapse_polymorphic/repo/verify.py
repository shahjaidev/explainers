...
def main() -> int:
    ...
    cols = taskdb.columns(conn, "comments")
    ...
    n = taskdb.as_int(taskdb.one(conn, "SELECT count(*) FROM comments"))
    ...
    for row in conn.execute("SELECT id FROM comments ORDER BY id"):
        parent = app.comment_parent(conn, row[0])
        if not parent or parent[0] not in ("post", "photo") or parent[1] is None:
            print(f"FAIL: app.comment_parent returned {parent!r} for comment {row[0]}")
            return 1
